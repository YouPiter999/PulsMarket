...
export default function BlogNorthCyprusPage() {
  ...
  useEffect(() => {
    const fetchArticles = async () => {
      try {
        const res = await fetch("/api/blog?region=north-cyprus");
        if (res.ok) {
      ...
      <main className="flex-grow pt-12 pb-12 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full relative z-10">
        <div className="text-center mb-12">
          <h1 className="text-4xl font-black text-white tracking-tight sm:text-5xl">
            Блог <span className="text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-indigo-400">Северного Кипра</span>
          </h1>
          <p className="mt-4 text-lg text-slate-400 max-w-2xl mx-auto font-medium">
            Всё о ТРСК: недвижимость в Искеле, жизнь в Гирне, авто и полезные советы.
          </p>
          
          <div className="mt-8 flex justify-center gap-2 sm:gap-4 overflow-x-auto pb-2 no-scrollbar">
            <Link href="/blog" className="px-6 py-2.5 rounded-full text-sm font-bold bg-[#121b2e]/60 text-slate-300 hover:bg-[#121b2e] border border-white/5 transition-all duration-300 whitespace-nowrap hover:border-purple-500/20">
              Все статьи
            </Link>
            ...
            <Link href="/blog/north-cyprus" className="px-6 py-2.5 rounded-full text-sm font-bold bg-gradient-to-r from-purple-600 to-indigo-600 text-white shadow-[0_0_20px_rgba(139,92,246,0.3)] border border-purple-500/30 whitespace-nowrap">
              Северный Кипр (ТРСК)
            </Link>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-20 bg-[#121b2e]/40 rounded-3xl border border-white/5 backdrop-blur-md">
            ...
            <h3 className="text-xl font-bold text-white">Статей про Северный Кипр пока нет</h3>
            <p className="text-slate-400 mt-2">Скоро мы добавим сюда полезный материал!</p>
          </div>
        ) : (
